...
# Earthquake Dataset class (as provided)
...
# def write_filtered_data_text_classification(args, train_dy_metrics):
#   """
...
#         if args.task_name in ["SNLI", "MNLI", "EcuadorEarthquake", "NepalBalanced", "NepalEarthquake", "VetMed"]:
...
def write_filtered_data(args, train_dy_metrics):
    """
    Filter data based on the given metric and write image paths and labels for image classification tasks.
    """

    # Save filtering configuration for reference
    argparse_dict = vars(args)
    with open(os.path.join(args.filtering_output_dir, "filtering_configs.json"), "w") as outfile:
        outfile.write(json.dumps(argparse_dict, indent=4, sort_keys=True) + "\n")
    ...
    # Sort scores by metric
    sorted_scores = train_dy_metrics.sort_values(by=[args.metric], ascending=is_ascending)

    # Original image directory path
    dataset = EarthquakeDataset(img_dir=args.data_dir)


    for fraction in [0.01, 0.05, 0.10, 0.1667, 0.25, 0.3319, 0.50, 0.75]:
        outdir = os.path.join(args.filtering_output_dir, f"cartography_{args.metric}_{fraction:.2f}", args.task_name)
        os.makedirs(outdir, exist_ok=True)

        # Number of samples to select based on fraction
        num_samples = int(fraction * len(sorted_scores))
        
        # Select hardest and/or easiest samples if both ends are required
        selected = sorted_scores.head(num_samples)
        if args.both_ends:
            hardest = sorted_scores.head(int(num_samples * 0.7))
            easiest = sorted_scores.tail(num_samples - len(hardest))
            selected = pd.concat([hardest, easiest])

        logger.info(f"Filtering {num_samples} samples for fraction {fraction:.2f}")

        # Iterate over selected records and copy images or save paths
        selection_iterator = tqdm.tqdm(range(len(selected)))
        image_paths_labels = []
        ...
        for idx in selection_iterator:
            ...
            selected_guids = selected.iloc[idx]["guid"]
            selected_label = dataset[int(selected_guids)]
            
            # Copy image to output directory (optional)
            #target_dir = os.path.join(outdir, f"class_{selected_label}")
            #os.makedirs(target_dir, exist_ok=True)
            #shutil.copy(selected_image_path, target_dir)

            # Alternatively, save image path and label
            image_paths_labels.append((selected_guids, selected_label))

        # Save image paths and labels to a CSV file
        output_csv = os.path.join(outdir, "filtered_images.csv")
        pd.DataFrame(image_paths_labels, columns=["image_path", "label"]).to_csv(output_csv, index=False)

        logger.info(f"Saved filtered image data to {output_csv}")
...
if __name__ == "__main__":
  ...
  parser.add_argument("--task_name",
                      "-t",
                      default="WINOGRANDE",
                      choices=("SNLI", "MNLI", "QNLI", "WINOGRANDE", "EcuadorEarthquake", "NepalBalanced", "NepalEarthquake", "VetMed"),
